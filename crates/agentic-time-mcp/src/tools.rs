...
pub const TOOLS: &[ToolDefinition] = &[
    ...
    ToolDefinition {
        ...
        description: "Fork the timeline for what-if analysis at a decision point",
        input_schema: r#"{"type":"object","properties":{"decision":{"type":"string","description":"The decision being made"},"alternatives":{"type":"array","items":{"type":"object","properties":{"label":{"type":"string"},"description":{"type":"string"}},"required":["label","description"]}}},"required":["decision","alternatives"]}"#,
    },
    ToolDefinition {
        name: "time_clone_race",
        description: "Create parallel exploration clones racing to find the best approach",
        input_schema: r#"{"type":"object","properties":{"approaches":{"type":"array","items":{"type":"string"},"description":"List of approaches to explore in parallel"}},"required":["approaches"]}"#,
    },
    ToolDefinition {
        name: "time_echo_detect",
        description: "Detect temporal echoes and ripple effects from an event",
        input_schema: r#"{"type":"object","properties":{"event":{"type":"string","description":"Description of the event to analyze ripple effects for"}},"required":["event"]}"#,
    },
    ToolDefinition {
        name: "time_deadline_prophecy",
        description: "Predict deadline outcomes based on current trajectory",
        input_schema: r#"{"type":"object","properties":{}}"#,
    },
    ToolDefinition {
        name: "time_causal_archaeology",
        description: "Excavate cause-effect chains from temporal history",
        input_schema: r#"{"type":"object","properties":{"problem":{"type":"string","description":"The problem to investigate"},"window_hours":{"type":"number","description":"How far back to look (hours)","default":168}},"required":["problem"]}"#,
    },
    ToolDefinition {
        name: "time_future_memory",
        description: "Precompute future context for an upcoming point in time",
        input_schema: r#"{"type":"object","properties":{"target_time":{"type":"string","format":"date-time","description":"The future time to prepare context for"}},"required":["target_time"]}"#,
    },
    ToolDefinition {
        name: "time_debt_analyze",
        description: "Analyze temporal debt from overdue and deferred items",
        input_schema: r#"{"type":"object","properties":{}}"#,
    },
    ToolDefinition {
        name: "time_gravity_detect",
        description: "Detect chrono-gravity wells pulling temporal items toward them",
        input_schema: r#"{"type":"object","properties":{}}"#,
    },
    ToolDefinition {
        name: "time_entanglement_create",
        description: "Create a temporal entanglement linking entities that must move together",
        input_schema: r#"{"type":"object","properties":{"entity_ids":{"type":"array","items":{"type":"string"},"description":"IDs of entities to entangle"},"entanglement_type":{"type":"string","enum":["sync","sequence","mirror","inverse","failure_propagation"],"default":"sync"},"gap_secs":{"type":"integer","description":"Gap in seconds (for sequence type)"}},"required":["entity_ids"]}"#,
    },
    ToolDefinition {
        name: "time_anomaly_scan",
        description: "Scan the temporal graph for anomalies and health issues",
        input_schema: r#"{"type":"object","properties":{}}"#,
    },
    ToolDefinition {
        name: "time_decay_reversal",
        description: "Analyze reversal options for a decaying value",
        input_schema: r#"{"type":"object","properties":{"id":{"type":"string","description":"Decay model ID to analyze reversal for"}},"required":["id"]}"#,
    },
    ToolDefinition {
        name: "time_dilation_analyze",
        description: "Analyze time dilation patterns and subjective time perception",
        input_schema: r#"{"type":"object","properties":{}}"#,
    },
    ToolDefinition {
        name: "time_jump_create",
        description: "Jump to a specific point in the project timeline",
        input_schema: r#"{"type":"object","properties":{"destination":{"type":"string","format":"date-time","description":"The point in time to jump to"}},"required":["destination"]}"#,
    },
    ToolDefinition {
        name: "time_anchor_create",
        description: "Create a temporal anchor capturing current state as a savepoint",
        input_schema: r#"{"type":"object","properties":{"name":{"type":"string","description":"Name for this anchor"},"reason":{"type":"string","description":"Why this anchor is being created"}},"required":["name","reason"]}"#,
    },
    ToolDefinition {
        name: "time_loop_detect",
        description: "Detect time loop patterns from repeated similar events",
        input_schema: r#"{"type":"object","properties":{}}"#,
    },
    ToolDefinition {
        name: "time_wormhole_create",
        description: "Connect two distant points in time with a causal wormhole",
        input_schema: r#"{"type":"object","properties":{"label_a":{"type":"string"},"time_a":{"type":"string","format":"date-time"},"description_a":{"type":"string"},"label_b":{"type":"string"},"time_b":{"type":"string","format":"date-time"},"description_b":{"type":"string"},"relationship":{"type":"string","description":"How these two time points are causally connected"}},"required":["label_a","time_a","description_a","label_b","time_b","description_b","relationship"]}"#,
    },
];
...
pub async fn handle_tool_call(
    ...
    match name {
        ...
        "time_timeline_fork" => {
            ...
            let result = engine
                .timeline_fork(decision, alternatives)
                .map_err(|e| e.to_string())?;
        ...
        "time_clone_race" => {
            ...
            let result = engine.clone_race(approaches).map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }

        "time_echo_detect" => {
            let event = require_str(&args, "event")?;
            let result = engine.echo_detect(event).map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }

        "time_deadline_prophecy" => {
            let result = engine.deadline_prophecy().map_err(|e| e.to_string())?;
            Ok(json!({
                "prophecies": serde_json::to_value(&result).map_err(|e| e.to_string())?,
                "count": result.len(),
            }))
        }

        "time_causal_archaeology" => {
            ...
            let result = engine
                .causal_archaeology(problem, window_secs)
                .map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }
        ...
        "time_debt_analyze" => {
            let result = engine.debt_analyze().map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }

        "time_gravity_detect" => {
            let result = engine.gravity_detect().map_err(|e| e.to_string())?;
            Ok(json!({
                "gravity_wells": serde_json::to_value(&result).map_err(|e| e.to_string())?,
                "count": result.len(),
            }))
        }

        "time_entanglement_create" => {
            let ids_val = args
                .get("entity_ids")
                .and_then(|v| v.as_array())
            ...
            let etype = match etype_str {
                ...
                "failure_propagation" => {
                    agentic_time::inventions::EntanglementType::FailurePropagation
                }
                other => return Err(format!("Unknown entanglement_type: {}", other)),
            };

            let result = engine
                .entanglement_create(entity_ids, etype)
                .map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }

        "time_anomaly_scan" => {
            let result = engine.anomaly_scan().map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }

        "time_decay_reversal" => {
            let id = require_id(&args, "id")?;
            let result = engine.decay_reversal(&id).map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }

        "time_dilation_analyze" => {
            let result = engine.dilation_analyze().map_err(|e| e.to_string())?;
            serde_json::to_value(&result).map_err(|e| e.to_string())
        }
        ...
        "time_loop_detect" => {
            let result = engine.loop_detect().map_err(|e| e.to_string())?;
            Ok(json!({
                "loops": serde_json::to_value(&result).map_err(|e| e.to_string())?,
                "count": result.len(),
            }))
        }

        "time_wormhole_create" => {
