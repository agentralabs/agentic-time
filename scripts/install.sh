#!/bin/bash
# AgenticTime — one-liner install script
...
#
# What it does:
#   1. Downloads agentic-time-mcp binary to ~/.local/bin/
...
# ── Constants ──────────────────────────────────────────────────────────
REPO="agentralabs/agentic-time"
BINARY_NAME="agentic-time-mcp"
SERVER_KEY="agentic-time"
...
download_binary() {
    ...
    local asset_name="agentic-time-${version_num}-${platform}.tar.gz"
    ...
    # Copy both binaries (atime CLI + MCP server)
    cp "${tmpdir}"/agentic-time-*/atime "${INSTALL_DIR}/atime" 2>/dev/null || true
    cp "${tmpdir}"/agentic-time-*/${BINARY_NAME} "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/atime" 2>/dev/null || true
...
install_from_source() {
    ...
    mkdir -p "${INSTALL_DIR}"
    cp "${cargo_bin}/atime" "${INSTALL_DIR}/atime"
    cp "${cargo_bin}/${BINARY_NAME}" "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/atime" "${INSTALL_DIR}/${BINARY_NAME}"
    echo "  Installed from source to ${INSTALL_DIR}/atime and ${INSTALL_DIR}/${BINARY_NAME}"
}
...
find_timeline() {
    ...
    for candidate in \
        "\${ATIME_FILE:-}" \
        "\${AGENTRA_ATIME_PATH:-}" \
        "\${repo_root}/.agentra/\${repo_slug}.atime" \
        "\${repo_root}/.timeline.atime" \
        "\$PWD/.timeline.atime" \
        "\$PWD/timeline.atime" \
        "\$HOME/.timeline.atime" \
        "\$HOME/.agentra/time/default.atime"; do
    ...
    printf '%s' "\${repo_root}/.agentra/\${repo_slug}.atime"
}
...
for arg in "\${args[@]}"; do
    case "\$arg" in
        ...
        -t|--timeline|--timeline=*) has_timeline=1 ;;
...
if [ "\$has_timeline" -eq 0 ]; then
    timeline_path="\$(find_timeline || true)"
    if [ -n "\$timeline_path" ]; then
        args=(--timeline "\$timeline_path" "\${args[@]}")
    fi
fi
...
print_post_install_next_steps() {
    ...
    if [ "$PROFILE" = "server" ]; then
        ...
        echo "  3. If artifacts were created on another machine, sync .atime/.amem/.acb/.avis files to this server." >&3
        echo "  4. Start MCP with auth, connect clients, then restart clients." >&3
        echo "  5. Optional feedback: open https://github.com/agentralabs/agentic-time/issues" >&3
    else
        echo "  2. Restart your MCP client/system so it reloads MCP config." >&3
        echo "  3. ${SERVER_KEY} now auto-detects local .atime files at startup when available." >&3
...
main() {
    ...
    finish_progress
    echo "Install complete: AgenticTime (${PROFILE})" >&3
    echo "" >&3
    echo "Done! Timeline defaults to .agentra/<project>.atime" >&3
